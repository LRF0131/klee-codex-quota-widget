...
- Right-click the Logo and drag the **界面缩放** (UI scale) slider directly inside the menu for a live 50%–200% adjustment. Use arrow keys for fine control or reset to 100%. The menu stays open while adjusting and saves when closed. Scaling follows system DPI and is capped to fit the taskbar; the Logo hit area scales with it. Menu colors follow the system theme.
