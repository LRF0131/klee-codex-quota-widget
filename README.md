...
- 运行时在 Windows 通知区域常驻后台图标，桌面快捷方式可用于手动重新启动。
...
3. 双击 `Install.cmd`。组件会立即启动，为当前 Windows 用户设置开机启动，并在实际桌面目录创建 `Codex Quota Widget` 快捷方式。
...
- 程序运行时，Windows 任务栏右侧的通知区域会出现 Codex 图标；如果没有直接显示，请在“显示隐藏的图标”面板中查看。
- 悬停图标可查看当前 `5h`、`7d` 额度和任务状态；双击会立即刷新，右键可打开完整菜单。
- 如果任务栏组件找不到或意外退出，双击桌面的 `Codex Quota Widget` 快捷方式即可重新启动。
- 若桌面快捷方式被删除，也可以运行 `%LOCALAPPDATA%\KleeCodexQuotaWidget\KleeCodexQuotaWidget.exe`。
- Windows 可能把新图标折叠到隐藏区域；可在系统的“其他系统托盘图标”设置中选择是否始终显示。

卸载程序会移除开机启动、程序文件和桌面快捷方式。额度缓存与日志仍会保留，便于需要时排查。
...
右键菜单按所在显示器的 DPI 和 Windows 文本大小重新测量，缩放百分比、按钮、提示和滑杆一起布局。空间不足时换行，菜单限制在屏幕工作区内；极窄屏幕配合大字号时，菜单文字会适当缩小以保留完整操作名称。切换显示器后会重新布局，小组件的自定义缩放不改变菜单大小。

1.4.2 修复了 175%（168 DPI）下的文字裁切。以下为 175%、文本大小 100% 的模拟渲染：

| 深色菜单 | 浅色菜单 |
| --- | --- |
| ![175% 深色菜单](docs/menu-175-dark.png) | ![175% 浅色菜单](docs/menu-175-light.png) |

菜单自检覆盖 128 组布局：100%–300% 的八档 DPI、100%/125%/150%/200% 文本大小、深浅主题和 800/1920 像素工作区宽度，并检查滑杆输入、恢复默认、控件边界及 DPI 往返。上述矩阵是模拟布局与绘制测试，当前电脑另做了菜单打开与重绘检查；尚未逐一验证所有实体显示器、远程桌面及第三方任务栏工具组合。
