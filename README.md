...
当 7d 剩余额度归零，且 Codex 官方接口确认账户至少有一次可用重置时，右侧显示：

```text
GPT  5h 31%  |  7d 0%  |  可重置 ×1
```
...
- 拖动 Logo：微调任务栏横向位置。
- 双击 Logo：立即刷新。
- 右键 Logo：刷新、打开监控源、恢复左下位置、切换开机自启动或退出。
- 文字和其他区域鼠标穿透。显示层采用逐像素 Alpha；Logo 的独立点击区域采用 1/255 Alpha，方便点击图案空隙。
...
- 右侧优先级：额度已恢复 → 7d 归零状态 → Tibo 预告 → 5h 低于 10% 的恢复时间 → 隐藏。
- 7d 归零时，有官方确认的重置机会则显示 `可重置 ×N`，否则显示已知的未来恢复日期；未知次数不等于零。
- 7d 未归零时不提示重置次数。5h 低于 10% 时显示恢复时间，跨天注明“明天”。
- Tibo 预告到点后显示“待确认”；此时若 5h 耗尽，优先显示其已知恢复时间。
- 恢复提示只触发一次，五分钟后消失；期间再次耗尽则提前结束。过期数据不显示额度相关状态。
...
- 界面使用 `UpdateLayeredWindow` 提交透明位图，文字使用灰度抗锯齿；图标动画在透明区域清除后重画。上次额度、恢复时间和重置次数在本机缓存。
